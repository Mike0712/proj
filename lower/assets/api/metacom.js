...
export class Metacom extends EventEmitter {
    ...
    message(data) {
        ...
        if (callId && args) {
            // For React
            if (this.reactCalls.has('message')) {
                const reactCallback = this.reactCalls.get('message');
                reactCallback(callType, args);
            }
...
class WebsocketTransport extends Metacom {
    async open() {
        ...
        socket.addEventListener('close', () => {
            ...
            if (this.reactCalls.has('close')) {
                this.reactCalls.get('close')();
            }
        });
        ...
        return new Promise((resolve) => {
            socket.addEventListener('open', () => {
                this.connected = true;
                if (this.reactCalls.has('open')) {
                    this.reactCalls.get('open')();
                }
                resolve();
            });
        });
